...
export default {
  data: new SlashCommandBuilder()
    ...
    .addBooleanOption(option => option
      .setName('hold')
      .setDescription('再生が終了するまで待つ このオプションを使用する場合、toとの併用が必須です')
    ),
  /**
  ...
  async execute (interaction) {
    ...
    interaction.reply({
      embeds: [new EmbedBuilder()
        .setTitle(queue.getSize() !== 0 ? `:file_cabinet: ${queue.estimatedDuration === 0 ? ':satellite: LIVE ONLY' : queue.durationFormatted} / ${queue.getSize()}曲` : null)
        .setDescription(`${isHolded ? ':label:' : ':track_next:'} ${t.title.length < 15 ? ` [${t.title}]` : `\n[${t.title}]`}(${t.url})\n:label: <@${t.requestedBy.id}>\n:timer: ${t.durationMS === 0 ? ':satellite: LIVE' : t.duration}`)
        .setColor(data.mutaoColor)
        .setAuthor({ name: to ? `\n${to}曲目にスキップ${isHolded ? '予定です' : 'しました'}` : null, iconURL: t.thumbnail })
      ]
    })
  }
}
