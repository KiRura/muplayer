...
import { ChatInputCommandInteraction, SlashCommandBuilder, EmbedBuilder, ChannelType, PermissionFlagsBits } from 'discord.js'
...
export default {
  ...
  async execute (interaction) {
    ...
    // 再生先のVCを取得
    const vc = interaction.options.getChannel('vc') || interaction.member.voice.channel || interaction.guild.members.me.voice.channel
    if (!vc) {
      return await interaction.reply({
        embeds: [new EmbedBuilder()
          .setTitle('再生先のVCがありません。')
          .setDescription('- 再生先のVCが指定されている\n- 実行者がVCに接続済み\n- BOTが既にVCに接続済み\n上記3つのいずれかを満たす必要があります。')
      ...
      return await interaction.reply({
        embeds: [new EmbedBuilder()
          ...
          .setDescription(`<#${vc.id}> に接続できません。権限を確認して下さい。`)
          .setColor(data.redColor)
        ],
        ephemeral: true
      })
    ...
    if (volume > 50 && !interaction.member.permissions.has(PermissionFlagsBits.Administrator)) volume = 50 // 鼓膜破壊を防ぐため
    ...
    if (!track.hasTracks()) {
      return interaction.reply({
        embeds: [new EmbedBuilder()
          ...
          .setDescription('URLの余計な部分や検索ワードを変えて再度お試し下さい。')
          .setColor(data.redColor)
        ]
      })
    }
    ...
    const queueNumber = currentQueue ? `${currentQueue.getSize() + 1}番目に追加｜キュー内合計: ${isUrl ? (currentQueue ? currentQueue.getSize() : 0) + track.tracks.length : (currentQueue ? currentQueue.getSize() : 0) + 1}曲` : '再生開始'
    ...
    if (!(query.startsWith('http://') || query.startsWith('https://'))) description = `${description}\n:mag_right: ${query.substring(0, 15)}${query.length > 15 ? '...' : ''}`
