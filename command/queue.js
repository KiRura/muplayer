  ...
  async execute(interaction) {
    ...
    const queueLength =
      queue.estimatedDuration + queue.currentTrack.durationMS === 0
        ? ":satellite: LIVE ONLY"
        : `:file_cabinet: ${functions.times(length)}`;
    const streamTime =
      queue.currentTrack.durationMS === 0
        ? "LIVE"
        : `${queue.node.getTimestamp().current.label} / ${queue.currentTrack.duration}`;
      ...
      embeds: [
        new EmbedBuilder()
          ...
          .setAuthor({
            ...
            name: `再生中: (${streamTime}) ${queue.currentTrack.title.length <= 20 ? queue.currentTrack.title : `${queue.currentTrack.title.substring(0, 20)}...`}`,
