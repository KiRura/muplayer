...
export default {
  data: new SlashCommandBuilder()
    .setName('seek')
    .setDescription('h: 時間・m: 分・s: 秒で指定します。空の場合はデフォルトで0になります。')
  ...
  async execute (interaction) {
    ...
    if (queue.currentTrack.durationMS < msec) {
      return interaction.reply({
        embeds: [new EmbedBuilder()
          .setTitle('あたおかナンバー')
          .setDescription('`' + queue.currentTrack.duration + '`より短い時間に指定する必要があります。\n指定された時間: `' + formatted + '`')
    ...
    await queue.node.seek(msec) ? interaction.followUp(`${formatted} に移動しました。`) : interaction.followUp('何故かできませんでした。')
  }
}
